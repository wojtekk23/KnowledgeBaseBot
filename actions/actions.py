...
def resolve_mention(tracker: Tracker) -> Text:
    ...
    if not listed_items: return None

    return listed_items[mention_solver.get(tracker.get_slot('mention'), 0)]
...
class ActionQueryEntities(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        slots = [
            ...
            SlotSet("listed_items", sorted(list(map(lambda x: to_str(x, entity_key), entities)))),
        ]
...
class ActionQueryAttribute(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        print(entity_type, key_attribute, name, attribute)
